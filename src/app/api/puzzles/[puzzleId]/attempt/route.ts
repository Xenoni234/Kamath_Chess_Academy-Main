...
export async function POST(request: NextRequest, context: { params: Promise<{ puzzleId: string }> }) {
  ...
  try {
    ...
    const body = (await request.json()) as { solved: boolean; timeTakenMs: number };
    ...
    const easinessFactor = body.solved ? Math.max(1.3, previousEaseFactor + 0.1) : previousEaseFactor;
    const intervalDays = body.solved ? (previousInterval <= 1 ? 6 : Math.round(previousInterval * easinessFactor)) : 1;
    const nextReviewAt = body.solved
    ...
    const data = {
      ...
      easinessFactor,
      intervalDays,
      repetition: body.solved ? (previous?.repetition ?? 0) + 1 : 0,
      nextReviewAt,
      reviewedAt: now,
    };
